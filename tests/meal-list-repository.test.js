...
test('饮食清单 repository 隔离旧购物车协议并补充菜谱 ID', async () => {
  ...
  const repository = createMealListRepository({
    async callFunction(name, data) {
      ...
      if (data.action === 'get') return { data: [{ foodId: 13, name: '麻婆豆腐', quantity: 1 }] }
      return { data: { count: 2, total: 36 } }
    }
  })

  assert.deepEqual(await repository.listItems(), [{ foodId: 13, dishId: 13, name: '麻婆豆腐', quantity: 1 }])
